...
What's in this repo
Prerequisites
Setup — Azure DevOps (~10 min)
Setup — Power Automate flow (~5 min)
Test it end to end
Troubleshooting
Production notes
Why this matters
...
Power Platform Build Tools extension installed in your DevOps org (free)
...
Commit commit-solution.yml to the root of your repo. One line to change:
...
⚠️ The gotcha that costs the most time: the useful fields live under OutputParameters, not InputParameters. See sample-trigger-payload.json for the real structure.
...
Make a small edit to a solution in dev (tweak a topic message in your agent)
Deploy to Test via your pipeline — approve if you have a gate
...
Open the commit's diff — your exact change, in plain text. That's your agent under source control. 🎉
...
pp-pipelineapproval — a pre-deployment approval flow on OnPreDeploymentStarted. Together they bookend the deployment: approval gates the deploy going in, auto-commit records the evidence coming out.
